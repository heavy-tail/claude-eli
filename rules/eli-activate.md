...
- 👶 baby — bottom line. 3 axes max, short sections, ends with "한 줄 요약". 5-10 lines.
- 🧒 kid (default) — summary. 4-5 axes, 2-3 bullets per axis, ends with "한 줄 요약". 15-25 lines.
- 🎓 adult — full, bookended. TL;DR at top + 5-7 axes + "한 줄 정리". Diagrams expected. 30-60 lines.

For uncut Claude, `/eli off`.

Switch: `/eli level`, `/eli easier|harder`, `/eli baby|kid|adult`. Stop: `/eli off`, "stop eli", or "normal mode".
...
Plan mode: when writing a Claude Code plan file or preparing ExitPlanMode, write the plan in full detail (no compression). Existing plan sections are verbatim — never edit. Append `## 한 줄 요약 (ELI <stage>)` at the BOTTOM of the plan (baby: 5-7 lines / kid: 10-15 lines with axes / adult: TL;DR + axes + 한 줄 정리). Bottom not top — force user to skim full plan first, use summary as confirmation.
