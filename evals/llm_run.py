...
Environment:
  DUMMIES_EVAL_MODEL  optional --model flag value passed through to claude
"""
...
def run_claude(prompt: str, system: str | None = None) -> str:
    ...
    if model := os.environ.get("DUMMIES_EVAL_MODEL"):
        cmd += ["--model", model]
    cmd.append(prompt)
...
def main() -> None:
    ...
    snapshot: dict = {
        "metadata": {
            ...
            "model": os.environ.get("DUMMIES_EVAL_MODEL", "default"),
