...
if (skillContent) {
  ...
  output =
    ...
    '- 👶 **baby** — "the simplest version I can act on". One recommended path (others mentioned, not detailed). 3-5 steps OR one analogy. 0-1 gotcha (only the absolute most likely trap for THIS question). Frame + answer often fused into one opening sentence. ≤3 distinct sections. Verification questions ("다 했어?" / "is this right?") in baby = yes/no + 1-2 lines why/caveat. No tables, no checklists, no §-number citations for verification questions specifically. Length follows TOPIC, not preceding context length. Translation > analogy: jargon (cold start / middleware / hydration / serverless 등) needs inline 일상어 gloss; if a sentence reads like raw Claude with technical terms intact, baby failed regardless of structure or analogy presence.\n' +
    '- 🧒 **kid** (DEFAULT) — "the recommended path + 1-2 things that\'ll bite first". Recommended path **flagged** ("처음이면 이거"), inline one-line gloss for unfamiliar terms. 1-2 gotchas, only those that apply NOW. ≤4 distinct sections. Path-flag is mandatory: when 2+ methods exist, flag the recommended one for THIS question\'s situation; never present equally (path-equality anti-pattern). Unfamiliar terms ALWAYS get inline gloss; bare jargon = kid failure.\n' +
    '- 🎓 **adult** — **lossless restructuring of raw**. Same content as raw Claude would answer, just transformed: frame at top + restructured body (tables, headers, emphasis, "pick X if Y" picks for raw\'s ambiguous trade-offs) + TL;DR at bottom. **No new content added** — no extra "흔한 실수" / "피해야 할 패턴" / "확인 절차" sections raw didn\'t include. Length budget: ~1.0-1.3x raw, not 2-3x. Self-check: "could every fact in this answer match raw\'s content?" — if you\'re adding new facts, that\'s tutorial creep, prune. Lossless: NEVER add sections raw didn\'t include (흔한 실수 / 피해야 할 패턴 / 확인 절차 / edge-case enumeration). 1.5x+ raw = tutorial creep, stop and prune.\n' +
    '- ✨ **auto** — Claude picks per question among baby/kid/adult. Signals: beginner cues ("explain like I\'m 5", jargon questions) → baby; "production-grade", "architecture", "trade-offs" → adult; Yes/No or simple how-to → kid; complex trade-offs → adult; when uncertain → kid. Don\'t default-kid: actually pick per question. Default-kid habit defeats auto\'s purpose.\n\n' +
